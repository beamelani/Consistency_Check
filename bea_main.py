...
# STL Consistency Checking (ver 0.2): versione locale che avevo fatto per sistemare il visiting
# Date: 20-04-2024
#
...
class STLVisitor:
    def visit(self, node):
        ...
        if isinstance(node, list):
            ...
            elif isinstance(node[0],list) and node[1] in {')'}:
                return self.visit(node[0])
            elif isinstance(node[0], str) and node[0] in {'!', 'G', 'F', '('}:
                if node[0] in {'!'}:
                    ...
                    return self.visit_open_parenthesis(node[0], node[1], node[2])
            ...
            if node in {')'}:
                return self.visit_close_parenthesis(node)
            else:
    ...
    def visit_identifier(self, identifier):
        ...
        print(f"Visiting Variable: {identifier}")
        return identifier
